...
struct ikdTree_PointType
{
    ...
    template<typename T>
    void from_eigen_vec( const T & pt )
    {
        x = pt(0);
        y = pt(1);
        z = pt(2);
    ...
    template<typename T>
    ikdTree_PointType( const T & pt, const long & index )
    {
        from_eigen_vec(pt);
        m_pt_idx = index;
    }
...
class KD_TREE
{
  ...
  public:
    ...
    int           Add_Point( PointType &PointToAdd, bool downsample_on );
    void          Add_Point_Boxes( vector< BoxPointType > &BoxPoints );
    void          Delete_Points( PointVector &PointToDel );
    int           Delete_Point_Boxes( vector< BoxPointType > &BoxPoints );
    void          flatten( KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type  = delete_point_storage_set::NOT_RECORD);
